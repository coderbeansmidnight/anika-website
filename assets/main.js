...
const CATEGORY_META = {
    ...
    research: { label: 'Research', color: 'var(--violet)' },
...
const CATEGORY_ORDER = ['work', 'education', 'research', 'skills', 'projects', 'leadership', 'awards'];
...
function renderResume() {
    ...
    if (!tabList || !panel)
        return;
    const usedCategories = CATEGORY_ORDER.filter((cat) => RESUME_ENTRIES.some((entry) => entry.category === cat));
    ...
    function renderPanel(category) {
        ...
        panel.innerHTML = entries
          ...
          <div class="resume-entry">
            ...
            .join('');
        Array.from(panel.querySelectorAll('.resume-entry-head')).forEach((head) => {
    ...
    Array.from(tabList.querySelectorAll('.resume-tab')).forEach((tab) => {
        tab.addEventListener('click', () => {
            const category = tab.dataset.category;
            Array.from(tabList.querySelectorAll('.resume-tab')).forEach((t) => {
